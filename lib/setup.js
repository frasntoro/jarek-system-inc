...
const { getRAM, checkOllamaStatus, askQuestion, saveConfig, loadConfig, removeOllamaModel, getDirectorySize } = require("./utils");
const os = require("os");
...
async function runSetup() {
  ...
  if (ollamaRunning) { console.log(chalk.green("  [OK] Ollama is running.")); config.ollama_ready = true; }
    ...
    if ((await askQuestion("Install Ollama now? (Y/N): ")).toLowerCase() === "y") {
      try {
        ...
        if (process.platform === "darwin") try { execSync("open -a Ollama", { stdio: "ignore" }); } catch(e) { if (fs.existsSync("/Applications/Ollama.app")) execSync("open /Applications/Ollama.app", { stdio: "ignore" }); }
        process.stdout.write(chalk.gray("  Waking up the AI Engine... "));
        for (let i = 0; i < 10; i++) { process.stdout.write(". "); ollamaRunning = await checkOllamaStatus(); if (ollamaRunning) break; await new Promise(r => setTimeout(r, 2000)); }
  ...
  printRow("ENGINE:", "Ollama /Applications/Ollama.app");
